...
## Aggiornamenti workflow V1 (operativo)

- Dataset camere aggiornato alla struttura reale (piano 0, piani 1-6 con esclusioni camere non esistenti).
- Vista **Cameriera**: lista principale mostra solo camere attive (`da_fare`, `in_corso`); le completate sono separate dietro toggle **Mostra completate**.
- Vista **Facchino**: lista principale mostra solo task attivi (`da_fare`, `in_corso`); i task `fatto` sono separati dietro toggle **Mostra completate**.
- Dashboard Supervisor: visione completa mantenuta, con camere e task raggruppati per stato (`da_fare`, `in_corso`, `completata`/`fatto`).
